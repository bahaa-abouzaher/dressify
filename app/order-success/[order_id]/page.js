...
async function page({ params }) {
  ...
  return (
    <>
      <ClearCartState />
      <div className="flex flex-col gap-4 justify-center items-center px-4 text-(--main-text)">
        ...
        <span className="text-(--gray-text) text-center xs:text-lg text-base">Your order number is <strong className="text-(--main-text)">{order_id}</strong></span>

        <Link 
          href={"/"}
          className="flex justify-center items-center h-11 w-45 rounded-2xl bg-(--orange-main) hover:bg-(--orange-secondary) focus:bg-(--orange-secondary) text-white"
        >
          Back to Homepage
        </Link>
      </div>
